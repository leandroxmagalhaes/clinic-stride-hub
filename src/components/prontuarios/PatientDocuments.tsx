...
function getFileIcon(fileType: string) {
  if (fileType.startsWith("image/")) return <FileImage className="h-5 w-5 text-primary" />;
  if (fileType === "application/pdf") return <FileText className="h-5 w-5 text-destructive" />;
...
export function PatientDocuments({ pacienteId, prontuarioId, clinicId }: PatientDocumentsProps) {
  ...
  const fetchDocuments = useCallback(async () => {
    ...
    if (!error && data) setDocuments(data as PatientDocument[]);
    setLoading(false);
  }, [pacienteId]);
  ...
  const validateFile = (file: File): string | null => {
    if (!ACCEPTED_TYPES.includes(file.type))
      return "Tipo de ficheiro não suportado. Use PDF, imagens, Word ou Excel.";
    if (file.size > MAX_FILE_SIZE)
      return `Ficheiro demasiado grande. Máximo ${formatBytes(MAX_FILE_SIZE)}.`;
    return null;
  };
      ...
      {!showForm ? (
        ...
        >
          ...
          <div className="flex flex-col items-center gap-2">
            ...
            <div>
              <p className="text-sm font-medium">Arraste um ficheiro ou clique para seleccionar</p>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                    <Badge className={cn("text-[10px] px-1.5 py-0 h-4 border-0", catInfo.color)}>
                      {catInfo.label}
                    </Badge>
