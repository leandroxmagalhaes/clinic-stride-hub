...
import { Loader2, Merge, X, Phone, Mail, Calendar, Hash } from "lucide-react";
...
export function DuplicatePatientsModal({ isOpen, onClose, patients, onMergeComplete }: DuplicatePatientsModalProps) {
  ...
  const handleConfirmMerge = async (pair: DuplicatePair) => {
    ...
    try {
      ...
      // 2. Update kept patient fields if needed
  ...
  const renderPatientSide = (patient: Patient) => (
                ...
                <Card key={pair.key}>
                  <CardContent className="p-4 space-y-3">
                    <div className="flex gap-4">
                      {renderPatientSide(pair.a)}
                      <div className="w-px bg-border shrink-0" />
                      {renderPatientSide(pair.b)}
                    </div>
                    ...
                    {!isMerging ? (
                      ...
                      <div className="border-t pt-3 space-y-3">
                        ...
                        <div className="flex gap-2 justify-end">
                          <Button variant="outline" size="sm" onClick={() => setMergingPairKey(null)} disabled={merging}>Cancelar</Button>
                          <Button size="sm" onClick={() => handleConfirmMerge(pair)} disabled={merging}>
                            {merging && <Loader2 className="h-3.5 w-3.5 mr-1 animate-spin" />}
                            Confirmar Mesclagem
                          </Button>
                        </div>
                      </div>
