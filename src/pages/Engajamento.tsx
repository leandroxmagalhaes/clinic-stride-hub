...
import { Gift, AlertTriangle, Star, Users, Zap } from "lucide-react";
...
export default function Engajamento() {
  ...
  useEffect(() => {
    if (!user) return;
    setIsLoading(true);
    fetchEngagementData().finally(() => setIsLoading(false));
  }, [user]);
      ...
      <div className="space-y-6">
        <div className="flex justify-end">
          <Button asChild variant="outline" size="sm">
            <Link to="/automacoes" className="flex items-center gap-2">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <BirthdayPanel patients={birthdayPatients} isLoading={isLoading} />
          <ChurnRiskPanel patients={churnRiskPatients} isLoading={isLoading} />
          <FeedbackPanel
            feedback={feedback}
            npsData={npsData}
            isLoading={isLoading}
