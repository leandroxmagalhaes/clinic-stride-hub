...
// ── Tipos de período ──────────────────────────────────────────────────────────
type PeriodType = "mes" | "trimestre" | "ano" | "custom";
  ...
  switch (type) {
    ...
    case "trimestre":
      return {
        ...
        label: `T${Math.ceil((ref.getMonth() + 1) / 3)} ${ref.getFullYear()}`,
      };
    case "ano":
...
const EMPTY_FORM: CreateLocalTransactionData = {
  ...
  recorrente: false,
  recorrente_dia: new Date().getDate(),
};
...
export default function Financeiro() {
  ...
  const goToPrevPeriod = () => {
    ...
    else if (periodType === "ano") setPeriodRef(new Date(periodRef.getFullYear() - 1, 0, 1));
  };
  ...
  // ── Dados locais (despesas + entradas avulsas) ────────────────────────────
  const [localTransactions, setLocalTransactions] = useState<LocalTransaction[]>([]);
  // ─────────────────────────────────────────────────────────────────────────

  // ── Modal de nova transacção ──────────────────────────────────────────────
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [defaultKind, setDefaultKind] = useState<TransactionKind>("saida");
  const [form, setForm] = useState<CreateLocalTransactionData>(EMPTY_FORM);
  const [deleteId, setDeleteId] = useState<string | null>(null);
  // ─────────────────────────────────────────────────────────────────────────

  // Carrega dados Supabase quando o período muda
  useEffect(() => {
    loadSupabaseData();
  }, [period.start.getTime(), period.end.getTime()]);

  // Carrega dados locais sempre que o período muda
  useEffect(() => {
  ...
  const stats = useMemo(
    () => FinancialTransactionService.getStats(period.start, period.end, packRevenue),
    [localTransactions, packRevenue, period.start.getTime(), period.end.getTime()],
  );
  // ─────────────────────────────────────────────────────────────────────────
  ...
  useEffect(() => {
    // Pack revenue por mês — simplificado: só usa o mês atual como referência
    // Em produção, buscaria do Supabase por mês
    const packByMonth: Record<string, number> = {};
    const currentKey = format(new Date(), "yyyy-MM");
    packByMonth[currentKey] = packRevenue;
  ...
  const openModal = (kind: TransactionKind) => {
    ...
    setForm({
      ...EMPTY_FORM,
      kind,
      category: kind === "entrada" ? "consulta_avulsa" : "renda",
    });
    setIsModalOpen(true);
  };
  ...
  const handleDelete = (id: string) => {
    // Remove sufixo de recorrente se necessário
    const baseId = id.split("_")[0] + "_" + id.split("_")[1] + "_" + id.split("_")[2];
  ...
  // Todas as transacções do período (locais + packs convertidos para display)
  const allTransactions = useMemo(() => {
    ...
    const packs = packTransactions.map((t) => ({
      ...
      category: "Pack de créditos",
      kind: "entrada" as TransactionKind,
      amount: t.monetary_value || 0,
      payment_method: t.payment_method || "",
      recorrente: false,
      source: "pack" as const,
    }));

    return [...local, ...packs].sort((a, b) => b.date.localeCompare(a.date));
  }, [localTransactions, packTransactions]);
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <Button variant="outline" size="icon" className="h-9 w-9" onClick={goToPrevPeriod}>
              ‹
            </Button>
            <Button variant="outline" size="icon" className="h-9 w-9" onClick={goToNextPeriod}>
              ›
            </Button>
            <Button variant="outline" size="sm" className="h-9 gap-1" onClick={loadSupabaseData}>
          ...
          {isLoading ? (
            ...
            <>
              <StatCard
                ...
                subtitle={`Packs + receitas avulsas`}
                icon={TrendingUp}
              />
              <StatCard
                title="Total Saídas"
              ...
              <StatCard
                ...
                value={String(kpis?.sessionsCompleted || 0)}
                subtitle={`Receita exec. ${formatCurrency(kpis?.executedRevenue || 0)}`}
                icon={Receipt}
              />
            </>
          )}
                  ...
                  <ResponsiveContainer width="100%" height={220}>
                    <BarChart
                      data={[
                        {
                          name: period.label,
                          "Vendas (Caixa)": kpis?.salesRevenue || 0,
                          "Executado (Competência)": kpis?.executedRevenue || 0,
                        },
                      ]}
              ...
              <CardContent>
                {allTransactions.length === 0 ? (
                  <div className="text-center py-12 text-muted-foreground">
                    ...
                    <p className="text-xs mt-1">Use os botões acima para registar entradas ou despesas</p>
                  </div>
                ) : (
                      ...
                      <TableBody>
                        {allTransactions.map((tx) => (
                          <TableRow key={tx.id}>
                            ...
                            <TableCell>
                              <Badge
                                variant={tx.kind === "entrada" ? "default" : "destructive"}
                                className={tx.kind === "entrada" ? "bg-green-600" : ""}
                              >
                                {tx.kind === "entrada" ? "Entrada" : "Saída"}
                              </Badge>
                            </TableCell>
                            <TableCell
                              className={`text-right font-semibold ${tx.kind === "entrada" ? "text-green-600" : "text-destructive"}`}
                            >
                              {tx.kind === "entrada" ? "+" : "-"}
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {packTransactions.map((tx) => {
                        const pm = PAYMENT_METHOD_LABELS[tx.payment_method || ""] || {
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="flex gap-2">
              <Button
                size="sm"
                variant={form.kind === "entrada" ? "default" : "outline"}
                className={form.kind === "entrada" ? "bg-green-600 hover:bg-green-700" : ""}
                onClick={() => setForm({ ...form, kind: "entrada", category: "consulta_avulsa" })}
              >
                Entrada
              </Button>
              <Button
                size="sm"
                variant={form.kind === "saida" ? "destructive" : "outline"}
                onClick={() => setForm({ ...form, kind: "saida", category: "renda" })}
              >
                Despesa
              </Button>
            </div>
            ...
            <div className="space-y-1">
              ...
              <Input
                placeholder={form.kind === "entrada" ? "Ex: Consulta Dr. Silva" : "Ex: Renda do mês de março"}
            ...
            <div className="flex items-center justify-between p-3 rounded-lg bg-muted/50">
              <div>
                ...
                <p className="text-xs text-muted-foreground">Repete todos os meses no mesmo dia</p>
              </div>
            ...
            {form.recorrente && (
              <div className="space-y-1">
                ...
                <Input
                  ...
                  min="1"
                  max="28"
                  value={form.recorrente_dia || 1}
                  onChange={(e) => setForm({ ...form, recorrente_dia: parseInt(e.target.value) || 1 })}
                />
              </div>
            )}
          </div>
          ...
          <DialogFooter className="gap-2">
            ...
            <Button
              ...
              className={form.kind === "entrada" ? "bg-green-600 hover:bg-green-700" : ""}
              variant={form.kind === "saida" ? "destructive" : "default"}
            >
              Guardar
            </Button>
          </DialogFooter>
