DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'process-custom-reminders-30m') THEN
    PERFORM cron.unschedule('process-custom-reminders-30m');
  END IF;
END $$;

SELECT cron.schedule(
  'process-custom-reminders-30m',
  '*/30 * * * *',
  $cmd$
  SELECT net.http_post(
    url := 'https://efzxtildgdawcplykbdc.supabase.co/functions/v1/process-custom-reminders',
